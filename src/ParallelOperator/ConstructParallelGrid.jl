...
function construct_grid(χ::Function,grid::GridMultiBlock{TT,DIM},z::Vector{TT};xmode=:stop,ymode=:preiod,interpmode=:nearest) where {TT,DIM}
    ...
    for I in eachgrid(grid)
        ...
        if interpmode == :nearest
            ix,iy = _remap_to_nearest_neighbours(grid,Pgrid.Bplane,sgi)
        ...
        if interpmode == :nearest
            ix,iy = _remap_to_nearest_neighbours(grid,Pgrid.Fplane,sgi)
...
"""
    _remap_to_nearest_neighbours
Remap the points generated by an x,y plane to their nearest neighbour in the full grid
"""
function _remap_to_nearest_neighbours(grid::GridMultiBlock,plane::ParGrid,subgridindex)
    ix = zeros(Int,size(plane.x))
    iy = zeros(Int,size(plane.y))
    for (I,J) in enumerate(subgridindex)
        # IND = LinearIndices(grid.Grids[J].gridx)
        ix[I] = Grid.nearestpoint(grid.Grids[J],(plane.x[I],plane.y[I]))
        iy[I] = Grid.nearestpoint(grid.Grids[J],(plane.x[I],plane.y[I]))
    end
    return ix,iy
end
