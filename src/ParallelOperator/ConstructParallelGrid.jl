...
function _subgrid_index(grid::GridMultiBlock,plane::ParGrid)
    ...
    # checkpt = (0.7071067811865477, 0.7071067811865476)
    for J in eachindex(plane.x)
        pt = (plane.x[J],plane.y[J])
        # nearpt = [pt[1], pt[2]]
        gridind = findgrid(grid,pt,mode=:nearest)
        if gridind == -1
            nearpt, _, gridind = nearestpoint(grid,pt)
            plane.x[J] = nearpt[1]
            plane.y[J] = nearpt[2]
            # if J == 961; @show "hi", pt, nearpt, J; end
        end
        containedgrid[J] = gridind


        # if (J == 961) & (plane.x[J]>0) & (plane.y[J]>0) ; @show "hi 2", plane.x[J],plane.y[J], J; end
        # if J == 931; plane.x[J] = 10.0; end
