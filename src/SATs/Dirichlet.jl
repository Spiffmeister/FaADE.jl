...
function generate_Dirichlet(SATD::SAT_Dirichlet,solver)
    ...
    let α = SATD.α, 
        ...
        if solver == :cgie
            ...
            CGTerm(cache::Array,RHS::Array,c::Array,::SATMode{:DataMode}) = 
...
function SAT_Dirichlet_implicit_data!(SAT::AbstractArray{T},::NodeType{:Left},RHS::AbstractArray{T},c::AbstractArray{T},
        α::Float64,τ::Float64,BD::AbstractVector,order::Int,loopaxis::Function) where T

    for (S,U,C) in zip(loopaxis(SAT),loopaxis(RHS),loopaxis(c))
        for i = 1:order
            S[i] += α*C[1]*BD[i]*U[1] #DₓᵀE₀
        end
...
function SAT_Dirichlet_implicit_data!(SAT::AbstractArray{T},::NodeType{:Right},RHS::AbstractArray{T},c::AbstractArray{T},
        α::Float64,τ::Float64,BD::AbstractVector,order::Int,loopaxis::Function) where {T}
    for (S,U,C) in zip(loopaxis(SAT),loopaxis(RHS),loopaxis(c))
        for i = 1:order
            S[end-order+i] += -α*C[end]*BD[i]*U[end] #DₓᵀEₙ
