...
function BoundaryDerivativeTranspose(::NodeType{:Left},order::Int,Δx::Float64)
    ...
    elseif order == 4
        ...
        return [-1152.0/289.0, 24.0/17.0, -192.0/731.0, -72.0/833.0]/Δx #E₀HDᵀ
    elseif order == 6
...
function BoundaryDerivativeTranspose(::NodeType{:Right},order::Int,Δx::Float64)
    if order == 2
        return [-0.5,1.0]/Δx
    elseif order == 4
        # return -[-24.0/17.0 * 48.0/49.0, 59.0/34.0 * 48.0/43.0, -4.0/17.0 * 48.0/59.0, -3.0/34.0 * 48.0/17.0]/Δx
        return [1152.0/833.0, -1416.0/731.0, 192.0/1003.0, 72.0/289.0]/Δx #EₙHDᵀ
    elseif order == 6
        return [-1.582533518939116, 2.033378678700676, -0.141512858744873, -0.450398306578272, 0.104488069284042, 0.036577936277544]/Δx
    end
end
